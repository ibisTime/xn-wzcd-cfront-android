...
import com.cdkj.baselibrary.utils.DataHelper;
    ...
    public void getInitDataDetails() {
        ...
        call.enqueue(new BaseResponseModelCallBack<CarLoanCalculatorActivityDetailsBean>(this) {
            ...
            protected void onSuccess(CarLoanCalculatorActivityDetailsBean data, String SucMessage) {
                ...
                double bb = data.getSfAmount() / data.getPrice();
                double shoufb = Double.parseDouble(new DecimalFormat("#.00").format(bb));
                mBinding.tvOnePay.setText(shoufb * 100 + "%");//首付
                mBinding.tvControlYear.setText((int) data.getPeriods() + "期");//还款期数
                mBinding.tvRato.setText(data.getSfRate() * 100 + "%");
                downPayments = shoufb;
                rate = data.getSfRate();
